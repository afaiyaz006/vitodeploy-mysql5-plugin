...
# The 8.0 client talks to a 5.7 server fine — wire protocol is backward
# compatible. The `[mysqldump] column-statistics=0` entry written to
# /root/.my.cnf below is what makes mysqldump 8 work against a 5.7 server
...
for i in $(seq 1 90); do
    ...
    if sudo docker exec mysql5 mysqladmin ping --silent >/dev/null 2>&1; then
        if [ "$i" -ge 5 ]; then
            ...
            echo "existing data dir at /var/lib/mysql5-data. The mysql:5.7 entrypoint"
...
# it lets the same binary back up a 5.7 server cleanly.
column-statistics=0
EOF
